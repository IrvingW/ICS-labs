...
# Description of my modifies
#
# Changes:
# 1. I've add the iaddl and leave instructions 
#
# 2. load-forwarding hazard introduced in class
#
# 3. reduce some mispredicted hazard penalties in the follow case:
#    opl
#    mrmovl
#    rmmovl
#    jle
...
int f_pc = [
	#reduce some mispredicted hazard penalties
	M_icode == IMRMOVL && !M_Cnd && E_icode == IRMMOVL && D_icode == IJXX && D_ifun == CLE: D_valP;
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && !(W_icode == IRMMOVL && M_ifun == CLE): M_valA;
...
bool instr_valid = f_icode in 
	{ INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  IOPL, IJXX, ICALL, IRET, IPUSHL, IPOPL, IIADDL, ILEAVE }; #M
...
bool need_regids =
	f_icode in { IRRMOVL, IOPL, IPUSHL, IPOPL, 
		     IIRMOVL, IRMMOVL, IMRMOVL, IIADDL }; #M
...
int d_srcA = [
	...
	D_icode in { ILEAVE } : REBP; #M
	1 : RNONE; # Don't need register
];

## What register should be used as the B source?
int d_srcB = [
	D_icode in { IOPL, IRMMOVL, IMRMOVL, IIADDL  } : D_rB; #M
...
int d_dstE = [
	D_icode in { IRRMOVL, IIRMOVL, IOPL, IIADDL } : D_rB; #M
	D_icode in { IPUSHL, IPOPL, ICALL, IRET, ILEAVE } : RESP; #M
...
int aluA = [
	...
	E_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IIADDL } : E_valC; #M
	E_icode in { ICALL, IPUSHL } : -4;
	E_icode in { IRET, IPOPL, ILEAVE } : 4; #M
...
int aluB = [
	E_icode in { IRMMOVL, IMRMOVL, IOPL, ICALL, 
		     IPUSHL, IRET, IPOPL, IIADDL, ILEAVE } : E_valB; #M
...
bool set_cc = E_icode in {IOPL,  IIADDL} && #M
	# State changes only during normal operation
	!m_stat in { SADR, SINS, SHLT } && !W_stat in { SADR, SINS, SHLT };

## Generate valA in execute stage
## To implement load frowarding, I modified the e_val 
int e_valA = [
	M_icode in { IMRMOVL, IPOPL, ILEAVE } && E_icode in { IRMMOVL, IPUSHL } && M_dstM == E_srcA :m_valM; #&& M_dstM != E_srcB : m_valM;
	1 : E_valA;
];
...
int mem_addr = [
	...
	M_icode in { IPOPL, IRET, ILEAVE } : M_valA; #M
...
# Comment: as I add the leave instr, it causes a hazard just like
#          load/use hazard , so I modified the following code.
#          (the use of %ebp will cause a load/use hazard)
bool F_bubble = 0;
bool F_stall =
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && !(W_icode == IMRMOVL && M_icode == IRMMOVL && E_ifun == CLE)) ||
	# Stalling at fetch while ret passes through pipeline
